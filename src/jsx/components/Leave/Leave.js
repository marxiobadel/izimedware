...
const Leave = () => {
            ...
            Cell: ({ row }) => (
                <Dropdown className="ms-auto c-pointer text-end">
                    ...
                    <Dropdown.Menu className="dropdown-menu-end" align="end">
                        <Dropdown.Item onClick={() => handleValidate(row.original)}>Valider</Dropdown.Item>
    ...
    useDocumentTitle('Congés');

    useEffect(() => {
        const controller = new AbortController();

        (() => {
            axiosInstance.get('leaves', {signal: controller.signal})
                .then(function({data}) {
        ...
        <>
            <div className="form-head align-items-center d-flex mb-sm-4 mb-3">
                <div className="me-auto">
                    <h2 className="text-black font-w600">Congés</h2>
                    <p className="mb-0">Liste des congés</p>
                </div>
                <div>
                    <Link to={"#"} className="btn btn-primary me-3" onClick={handleAdd}>+ Nouveau congé</Link>
                </div>
            </div>
            <ToastContainer />
            <Row>
