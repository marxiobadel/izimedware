...
const Slot = () => {
    ...
    useDocumentTitle('Créneaux horaires');

    useEffect(() => {
        const controller = new AbortController();

        (() => {
            axiosInstance.get('slots', {signal: controller.signal})
                .then(function({data}) {
                    setSlots([...data.slots]);
        ...
        <>
            <div className="form-head align-items-center d-flex mb-sm-4 mb-3">
                <div className="me-auto">
                    <h2 className="text-black font-w600">Créneaux horaires</h2>
                    <p className="mb-0">Liste des créneaux horaires</p>
                </div>
                <div>
                    <Link to={"#"} className="btn btn-primary me-3" onClick={handleAdd}>+ Nouveau créneau</Link>
                </div>
            </div>
            <ToastContainer />
            <Row>
